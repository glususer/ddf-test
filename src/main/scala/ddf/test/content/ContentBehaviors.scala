package ddf.test.content

import java.io.File
import java.util

import ddf.test.{BaseBehaviors, Loader}
...
trait ContentBehaviors extends BaseBehaviors {
  ...
  def ddfWithAddressing(implicit l: Loader): Unit = {
    ...
    it should "load data from file using loadTable" in {
      ...
      val ddf = l.ddfManager.loadFile(filePath, ",")
      ddf.getColumnNames should have size 29

    }

    it should "be addressable via name" in {
      l.ddfManager.getDDFByName(ddf.getName) should be(ddf)
    }
  }
  ...
  def ddfWithPersistenceHandler(implicit l: Loader): Unit = {
    it should "hold namespaces correctly" in {
      val manager: DDFManager = DDFManager.get(l.engine,l.ddfManager.getDataSourceDescriptor)
